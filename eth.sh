...
# overclocking and reducing power limit on GTX 1060 and GTX 1070
    ...
    exec 1>&3
    exec 2>&4 
    if [ -e /.driver_complete ] || grep -E "Coolbits.*28" /etc/X11/xorg.conf 1> /dev/null
    then
        :
    else
...
# Test for 60 minutes
    ...
    #     read -d "\0" -a user_array < <(who)
    #     rm -rf /setupethminer
    #     timeout 60m ethminer -S asia1.ethermine.org:4444 -O 0xad57c5b2d1c1c080986c127c2edf13be24317e93.mcm1 --farm-recheck 2000 -SC 2 -U --cuda-parallel-hash 4 --cuda-grid-size 8192 --cuda-block-size 256 -v 9 2>&1
    #fi
...
# Automatic startup with provided wallet address
    ...
    then
       ...
       #timeout 15m ethminer -S asia1.ethermine.org:4444 -O 0xad57c5b2d1c1c080986c127c2edf13be24317e93.mcm1 --farm-recheck 2000 -SC 2 -U --cuda-parallel-hash 4 --cuda-grid-size 8192 --cuda-block-size 256 -v 9 2>&1
	   
       wallet="$(cat /.wallet_provided)"
       printf "%s\n\n" "starting your miner at address $wallet"
       timeout 24h ethminer -S asia1.ethermine.org:4444 -O $wallet --farm-recheck 2000 -SC 2 -U --cuda-parallel-hash 4 --cuda-grid-size 8192 --cuda-block-size 256 -v 9 2>&1
       if [ "$?" -eq 0 ]
       then
       systemctl reboot
       else
